 ...
 * Builds real work_auction spend txs.
 * Default is UNSIGNED CBOR (Ryan signs). --submit is the only sign+submit path.
 * Keep WALLET_SEED_PHRASE off agent boxes.
 * Mutations look up on-chain RequestDatum/BidDatum/EscrowDatum. Not deployment.json chainIndex.
 * Honest BLOCKED if Blockfrost, Standard pNFT, script ref, factory UTxO, or on-chain datum is missing.
 */
...
function impactListLen(imp) {
  if (!imp) return 0;
  if (Array.isArray(imp)) return imp.length;
  if (Array.isArray(imp.list)) return imp.list.length;
  if (Array.isArray(imp.fields)) return imp.fields.length;
  return 0;
}

function decodeAllScriptDatums(utxos, deserializeDatum) {
  const decoded = [];
  for (const u of utxos) {
    ...
    if (d) decoded.push({ utxo: u, raw, txHash: u.input.txHash, outputIndex: u.input.outputIndex, ...d });
  }
  return decoded;
}

function requireOnChain(rows, pred, log, label) {
  const hit = rows.find(pred);
  if (!hit) {
    blocked(log, `${label} not found as on-chain work_auction datum. Not using deployment.json chainIndex.`);
    process.exit(1);
  }
  return hit;
}
...
export async function runChain(ctx) {
  ...
  const unsigned = hasFlag('--unsigned') || !hasFlag('--submit');
  if (unsigned && hasFlag('--submit')) {
    log.warn('--unsigned wins over --submit. Printing CBOR.');
  }
  if (!unsigned) {
    log.warn('--submit: will sign+submit if seed is present. Keep seed off agent boxes.');
  } else {
    log.info('Default UNSIGNED. Pass --submit only on a machine Ryan controls.');
  }
  ...
  const { BlockfrostProvider, MeshWallet, MeshTxBuilder, deserializeAddress, deserializeDatum } = await import('@meshsdk/core');
  const provider = new BlockfrostProvider(CONFIG.blockfrostKey);
  let wallet = null;
  ...
  const onChain = decodeAllScriptDatums(scriptUtxos, deserializeDatum);
  ...
  if (hasFlag('--bid')) {
    ...
    if (!jobId) { log.error('--job required'); process.exit(1); }
    const request = requireOnChain(onChain, (r) => r.kind === 'request' && r.requestId === jobId, log, `RequestDatum ${jobId}`);
    ...
    txBuilder.readOnlyTxInReference(request.txHash, request.outputIndex ?? 0);
    await spendFactory({ txBuilder, factoryUtxo, ref: resolved.ref, scriptAddress: resolved.address, datum, redeemer });
  ...
  if (hasFlag('--accept-bid') || hasFlag('--accept')) {
    ...
    if (!bidId) { log.error('--bid required'); process.exit(1); }
    const bid = requireOnChain(onChain, (r) => r.kind === 'bid' && r.bidId === bidId, log, `BidDatum ${bidId}`);
    const request = requireOnChain(onChain, (r) => r.kind === 'request' && r.requestId === bid.requestId, log, `RequestDatum ${bid.requestId}`);
    if (request.requester !== pnft.pnftId) { log.error('Only requester can accept'); process.exit(1); }
    if (impactListLen(request.expectedImpacts) < 1) {
      blocked(log, 'AcceptBid: on-chain RequestDatum expectedImpacts length < 1. Validator requires >= 1. Will not write an empty list.');
      process.exit(1);
    }
    const escrowId = `escrow_${crypto.randomBytes(8).toString('hex')}`;
    const workType = request.workTypeConstr || encodeWorkType(request.workType || 'construction', request.phase || 'roofing', WORK_TYPES);
    ...
    const datum = encodeEscrowDatum({
      ...
      asset: request.asset || '', workType, amount: bid.bidAmount, expectedImpacts: request.expectedImpacts, deadline: request.workDeadline,
    });
  ...
  if (hasFlag('--start') || hasFlag('--complete') || hasFlag('--confirm')) {
    ...
    if (!jobId) { log.error('--job required'); process.exit(1); }
    const request = requireOnChain(onChain, (r) => r.kind === 'request' && r.requestId === jobId, log, `RequestDatum ${jobId}`);
    const escrow = requireOnChain(onChain, (r) => r.kind === 'escrow' && r.requestId === jobId, log, `EscrowDatum for ${jobId}`);
    if (impactListLen(escrow.expectedImpacts) < 1 && impactListLen(request.expectedImpacts) < 1) {
      blocked(log, 'On-chain expectedImpacts length < 1. Validator requires >= 1. Will not write an empty list.');
      process.exit(1);
    }
    const expectedImpacts = impactListLen(escrow.expectedImpacts) >= 1 ? escrow.expectedImpacts : request.expectedImpacts;
    const workType = escrow.workTypeConstr || request.workTypeConstr || encodeWorkType(request.workType || 'construction', 'roofing', WORK_TYPES);
    ...
    const datum = encodeEscrowDatum({
      ...
      requester: request.requester, worker: escrow.worker || request.worker, asset: request.asset || '',
      workType, amount: escrow.amount, expectedImpacts, deadline: escrow.deadline || request.workDeadline,
    });
    await spendFactory({ txBuilder, factoryUtxo, ref: resolved.ref, scriptAddress: resolved.address, datum, redeemer });
