...
// SEED FUNCTIONS
// =============================================================================
async function createBioregion(lucid, bioregion) {
    console.log(`Creating bioregion: ${bioregion.name}`);
    // Encode bioregion datum using Constr for proper CBOR
    const indexValue = (val, conf) => new Constr(0, [BigInt(val), 0n, BigInt(conf)]);
    const healthScore = Math.round(Object.values(bioregion.indices).reduce((a, b) => a + b, 0) /
    ...
    const tx = await lucid
        .newTx()
        .payToContract(deployment.contracts?.bioregion || 'addr_test1TODO', { inline: datum }, { lovelace: 2000000n })
        .complete();
...
async function createDevelopmentPool(lucid) {
    ...
    const tx = await lucid
        .newTx()
        .payToAddress(deployment.contracts?.treasury || 'addr_test1TODO', { lovelace: 100000000n } // 100 ADA for pool
    )
        .complete();
    const signed = await tx.sign().complete();
    const txHash = await signed.submit();
...
async function createTestOfferings(lucid) {
...
async function createTestNeeds(lucid) {
...
async function createRegistryEntries(lucid) {
...
// =============================================================================
async function initializeTreasury(lucid) {
    ...
    try {
        ...
        const treasuryDatum = Data.to(new Constr(0, [
            400000000000000000n, // tokens_remaining (400B with 6 decimals)
            0n, // tokens_distributed
            0n, // ada_reserves
            0n, // btc_reserves
            1000000n, // ada_usd_rate (micro-USD, $1 per ADA initial)
        ]));
        // Seed treasury with initial ADA reserve
        const tx = await lucid
            .newTx()
            .payToContract(treasuryAddress, { inline: treasuryDatum }, { lovelace: 1000000000n } // 1000 ADA initial reserve
        )
            .complete();
        const signed = await tx.sign().complete();
        const txHash = await signed.submit();
...
//# sourceMappingURL=seed-testnet.js.map