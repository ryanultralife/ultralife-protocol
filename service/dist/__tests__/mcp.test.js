...
describe('MCP Tool Definitions', () => {
    ...
    describe('Tool Categories', () => {
        it('should have information tools', () => {
            const infoTools = EXPECTED_TOOLS.filter((t) => t === 'get_ultralife_info' || t === 'get_protocol_stats');
            expect(infoTools.length).toBe(2);
        });
        it('should have pNFT tools', () => {
            const pnftTools = EXPECTED_TOOLS.filter((t) => t.includes('pnft') && !t.includes('bucket'));
        ...
        it('should have treasury/bonding curve tools', () => {
            const treasuryTools = EXPECTED_TOOLS.filter((t) => t.includes('token_price') ||
                t.includes('purchase') ||
                t.includes('founder') ||
                t.includes('treasury'));
        ...
        it('should have marketplace tools', () => {
            const marketTools = EXPECTED_TOOLS.filter((t) => t.includes('offering') || t.includes('need'));
            expect(marketTools.length).toBeGreaterThanOrEqual(4);
        });
        it('should have collective tools', () => {
            const collectiveTools = EXPECTED_TOOLS.filter((t) => t.includes('collective'));
...
describe('Bonding Curve Logic', () => {
    describe('Price Calculation', () => {
        it('should start at floor price', () => {
            const totalSupply = 400000000000000000n; // 400B with 6 decimals
            ...
            const currentPrice = distributedRatio > 0 ? distributedRatio : floorPrice;
            expect(currentPrice).toBeCloseTo(floorPrice);
        });
        it('should increase price as more tokens are distributed', () => {
            const totalSupply = 400000000000000000n;
...
describe('Spending Bucket Templates', () => {
    const templates = {
        daily_spending: {
            allocation: 50000000n,
            period: 'Daily',
            rollover: true,
            maxBalance: 200000000n,
        },
        weekly_groceries: {
            allocation: 150000000n,
            period: 'Weekly',
            rollover: false,
            maxBalance: 150000000n,
        },
        monthly_bills: {
            allocation: 500000000n,
            period: 'Monthly',
            rollover: false,
            maxBalance: 500000000n,
        },
        emergency_fund: {
            allocation: 100000000n,
            period: 'Monthly',
            rollover: true,
            maxBalance: 5000000000n,
        },
        savings_goal: {
            allocation: 200000000n,
            period: 'Monthly',
            rollover: true,
            maxBalance: 10000000000n,
        },
        allowance: {
            allocation: 10000000n,
            period: 'Daily',
            rollover: false,
            maxBalance: 10000000n,
        },
        business_expense: {
            allocation: 1000000000n,
            period: 'Monthly',
            rollover: true,
            maxBalance: 3000000000n,
        },
    };
    ...
    it('should have emergency fund with high max balance', () => {
        expect(templates.emergency_fund.maxBalance).toBeGreaterThan(templates.daily_spending.maxBalance);
    });
    it('should have allowance as smallest allocation', () => {
        const allocations = Object.values(templates).map((t) => t.allocation);
        const minAllocation = allocations.reduce((min, a) => (a < min ? a : min), allocations[0]);
