 ...
 */
import { Lucid, Blockfrost, Data, fromText, fromHex, Constr, } from 'lucid-cardano';
// =============================================================================
...
export class UltraLifeTxBuilder {
    ...
    async initialize() {
        this.lucid = await Lucid.new(new Blockfrost(`https://cardano-${this.config.network}.blockfrost.io/api`, this.config.blockfrostApiKey), this.config.network === 'mainnet' ? 'Mainnet' : 'Preprod');
    ...
    async buildMintPnft(params) {
        ...
        const tx = await this.lucid
            .newTx()
            .mintAssets({ [policyId + assetName]: 1n }, redeemer)
            .readFrom([refScript])
            .payToContract(this.config.contracts.pnft_spend, { inline: datum }, { [policyId + assetName]: 1n })
            // Bootstrap grant: 50 tokens
            .payToAddress(params.userAddress, { [this.config.contracts.token_policy]: 50000000n })
            .complete();
        return {
            tx,
            summary: {
    ...
    async buildUpgradePnft(params) {
        ...
        if (!currentDatum)
            throw new Error('pNFT not found');
        // Find UTxO with pNFT
        const pnftUtxo = await this.findPnftUtxo(params.pnftId);
        if (!pnftUtxo)
            throw new Error('pNFT UTxO not found');
        ...
        const tx = await this.lucid
            ...
            .readFrom([refScript])
            .payToContract(this.config.contracts.pnft_spend, { inline: newDatum }, pnftUtxo.assets)
            .complete();
        return {
            tx,
            summary: {
    ...
    async buildCreateOffering(params) {
        ...
        if (!pnft)
            throw new Error('Offerer pNFT not found');
        if (pnft.level === 'Basic')
            throw new Error('Standard verification required');
        ...
        const tx = await this.lucid
            .newTx()
            .readFrom([refScript])
            .payToContract(this.config.contracts.marketplace, { inline: datum }, { lovelace: 2000000n } // Min UTxO
        )
            .complete();
        return {
            tx,
    ...
    async buildAcceptOffering(params) {
        ...
        const tx = await this.lucid
            ...
            .payToContract(this.config.contracts.work_auction, { inline: agreementDatum }, { [this.config.contracts.token_policy]: params.payment, lovelace: 2000000n })
            .complete();
        return {
            tx,
            summary: {
    ...
    async buildCreateCollective(params) {
        ...
        if (!pnft)
            throw new Error('Founder pNFT not found');
        if (pnft.level === 'Basic')
            throw new Error('Standard verification required');
        ...
        const tx = await this.lucid
            .newTx()
            .readFrom([refScript])
            .payToContract(this.config.contracts.collective, { inline: datum }, { lovelace: 2000000n })
            .complete();
        return {
            tx,
            summary: {
    ...
    async buildAddCollectiveMember(params) {
        ...
        if (!newMemberPnft)
            throw new Error('New member pNFT not found');
        ...
        if (!collectiveUtxo)
            throw new Error('Collective UTxO not found');
        const tx = await this.lucid
            .newTx()
            .collectFrom([collectiveUtxo], redeemer)
            .readFrom([refScript])
            .payToContract(this.config.contracts.collective, { inline: newDatum }, collectiveUtxo.assets)
            .complete();
        return {
            tx,
            summary: {
    ...
    async buildTransferTokens(params) {
        ...
        if (!senderData)
            throw new Error('Sender pNFT not found');
        if (!recipientData)
            throw new Error('Recipient pNFT not found');
        ...
        const tx = await this.lucid
            ...
            .readFrom(refScripts)
            // Transfer tokens
            .payToAddress(params.recipientAddress, { [this.config.contracts.token_policy]: params.amount })
            // Create transaction record
            .payToContract(this.config.contracts.records, { inline: recordDatum }, { lovelace: 2000000n })
            .complete();
        return {
            tx,
            summary: {
    ...
    async buildPurchaseFromPool(params) {
        ...
        const tokenAmount = params.adaAmount * 100n / 1000000n * 1000000n; // Convert to token units
        const tx = await this.lucid
            .newTx()
            // Send ADA to treasury
            .payToAddress(this.config.contracts.treasury, { lovelace: params.adaAmount })
            // Receive tokens (this would be handled by a pool contract in production)
            .payToAddress(params.buyerAddress, { [this.config.contracts.token_policy]: tokenAmount })
            .complete();
        return {
            tx,
            summary: {
                ...
                costs: { ada: `${params.adaAmount / 1000000n} ADA` },
    ...
    async buildCreateBucket(params) {
        ...
        const config = this.getBucketConfigFromTemplate(params.template, bucketId, params.name, params.allocation, params.period, params.rollover, params.maxBalance);
        // Build datum using Constr encoding
        ...
        if (initialFunding > 0n) {
            txBuilder.payToContract(this.config.contracts.spending_bucket, { inline: datum }, {
                [this.config.contracts.token_policy]: initialFunding,
                lovelace: 2000000n
            });
        }
        else {
            txBuilder.payToContract(this.config.contracts.spending_bucket, { inline: datum }, { lovelace: 2000000n });
        }
    ...
    async buildFundBucket(params) {
        ...
        if (!bucketUtxo)
            throw new Error('Bucket UTxO not found');
        const redeemer = Data.to(new Constr(1, [fromHex(params.bucketId), params.amount]));
        const tx = await this.lucid
            .newTx()
            .collectFrom([bucketUtxo], redeemer)
            .readFrom([refScript])
            .payToContract(this.config.contracts.spending_bucket, { inline: bucketUtxo.datum }, {
            ...bucketUtxo.assets,
            [this.config.contracts.token_policy]: (bucketUtxo.assets[this.config.contracts.token_policy] || 0n) + params.amount
        })
            .complete();
        return {
            tx,
    ...
    async buildSpendBucket(params) {
        ...
        const tx = await this.lucid
            ...
            .payToContract(this.config.contracts.spending_bucket, { inline: bucketUtxo.datum }, // Would update balance
        {
            ...bucketUtxo.assets,
            [this.config.contracts.token_policy]: (bucketUtxo.assets[this.config.contracts.token_policy] || 0n) - params.amount
        })
    ...
    async buildTransferBetweenBuckets(params) {
        ...
        const tx = await this.lucid
            ...
            .collectFrom([bucketUtxo], redeemer)
            .readFrom([refScript])
            .payToContract(this.config.contracts.spending_bucket, { inline: bucketUtxo.datum }, // Would update bucket balances
        bucketUtxo.assets)
            .complete();
        return {
            tx,
    ...
    getBucketConfigFromTemplate(template, bucketId, name, allocation, period, rollover, maxBalance) {
        const templates = {
            daily_spending: { allocation: 50000000n, period: 'Daily', rollover: true, maxBalance: 200000000n, minBalance: 0n },
            weekly_groceries: { allocation: 150000000n, period: 'Weekly', rollover: false, maxBalance: 150000000n, minBalance: 0n },
            monthly_bills: { allocation: 500000000n, period: 'Monthly', rollover: false, maxBalance: 500000000n, minBalance: 0n },
            emergency_fund: { allocation: 100000000n, period: 'Monthly', rollover: true, maxBalance: 5000000000n, minBalance: 1000000000n },
            savings_goal: { allocation: 200000000n, period: 'Monthly', rollover: true, maxBalance: 10000000000n, minBalance: 0n },
            allowance: { allocation: 10000000n, period: 'Daily', rollover: false, maxBalance: 10000000n, minBalance: 0n },
            business_expense: { allocation: 1000000000n, period: 'Monthly', rollover: true, maxBalance: 3000000000n, minBalance: 0n },
            custom: { allocation: allocation || 50000000n, period: period || 'Daily', rollover: rollover ?? true, maxBalance: maxBalance || 200000000n, minBalance: 0n },
        };
        return templates[template] || templates.custom;
    }
    encodePeriod(period) {
        const periods = { Daily: 0, Weekly: 1, Monthly: 2, Quarterly: 3, Yearly: 4 };
        return new Constr(periods[period] || 0, []);
    }
    async findBucketUtxo(pnftId) {
        if (!this.lucid)
            throw new Error('Builder not initialized');
    ...
    async findPnftUtxo(pnftId) {
        if (!this.lucid)
            throw new Error('Builder not initialized');
        const assetId = this.config.contracts.pnft_policy + pnftId;
        const utxos = await this.lucid.utxosAtWithUnit(this.config.contracts.pnft_spend, assetId);
        return utxos.length > 0 ? utxos[0] : null;
    }
    async findCollectiveUtxo(collectiveId) {
        if (!this.lucid)
            throw new Error('Builder not initialized');
    ...
    encodeVerificationMethod(method) {
        const typeIndex = {
            SelfReported: 0,
